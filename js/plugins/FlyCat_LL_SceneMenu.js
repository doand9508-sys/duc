 ...
 * @type variable
 * 
 * @param gongdeVariable
 * @text 功德变量
 * @desc 功德变量
 * @type variable
 * 
 * @param jingqiVariable
 * @text 精气进度变量
 * @desc 精气进度变量
 * @type variable
 * 
...
FlyCat.LL_SceneMenu.fqNode = String(FlyCat.LL_SceneMenu.parameters['fqNode']);
FlyCat.LL_SceneMenu.hyNode = String(FlyCat.LL_SceneMenu.parameters['hyNode']);
FlyCat.LL_SceneMenu.jsNode = String(FlyCat.LL_SceneMenu.parameters['jsNode']);
FlyCat.LL_SceneMenu.bzNode = String(FlyCat.LL_SceneMenu.parameters['bzNode']);
FlyCat.LL_SceneMenu.kzNode = String(FlyCat.LL_SceneMenu.parameters['kzNode']);
...
FlyCat.LL_SceneMenu.peopleStateYw = String(FlyCat.LL_SceneMenu.parameters['peopleStateYw']);
...
FlyCat.LL_SceneMenu.ywSwitch = Number(FlyCat.LL_SceneMenu.parameters['ywSwitch']);
FlyCat.LL_SceneMenu.jscSwitch = Number(FlyCat.LL_SceneMenu.parameters['jscSwitch']);
FlyCat.LL_SceneMenu.huaiyunSwitch = Number(FlyCat.LL_SceneMenu.parameters['huaiyunSwitch']);
...
FlyCat.LL_SceneMenu.hyMmonthVariable = Number(FlyCat.LL_SceneMenu.parameters['hyMmonthVariable']);

FlyCat.LL_SceneMenu.gongdeVariable = Number(FlyCat.LL_SceneMenu.parameters['gongdeVariable']);
FlyCat.LL_SceneMenu.jingqiVariable = Number(FlyCat.LL_SceneMenu.parameters['jingqiVariable']);
...
Game_Party.prototype.gainDurability = function () {
    ...
    if (!$gameSystem._armorDurability[armorId]) {
        $gameSystem._armorDurability[armorId] = 0;
    };
...
PluginManager.registerCommand('FlyCat_LL_SceneMenu', 'setHyText', args => {
    $gameSystem._hyPingJiaText = args.text;
});
/////////////////////称谓检测///////////////////////
...
Scene_Map.prototype.createDisplayObjects = function () {
    ...
    if ($gameSystem._armorDurability) {
        if ($gameSystem._armorDurability.length < $dataArmors.length) {
            for (let i = $gameSystem._armorDurability.length; i < $dataArmors.length; i++) {
                // if ($dataArmors[i] && $dataArmors[i].meta.菜单立绘换装) {
                //     $gameSystem._armorDurability[i] = 0;
                // }
                // else {
...
Game_Party.prototype.changeEquipPicture = function (item, actor) {
    ...
    // }
    // else {
    //     if (!actor._lastCharacterName) {
    //         actor._lastCharacterName = $dataActors[actor._actorId].characterName;
    //         actor._lastCharacterIndex = $dataActors[actor._actorId].characterIndex;
    //     }
    //     actor.setCharacterImage(actor._lastCharacterName, actor._lastCharacterIndex);
    // }
    if (hzItem && hzItem.meta.菜单立绘换装) {
        const lv = $we.breakClothLv(hzItem);
        if (lv == 0) {
            $gameSystem._menuActorPicture = '';
        } else {
            const hzImg = hzItem.meta.菜单立绘换装 + lv;
    ...
    // }
    if (hzItem && hzItem.meta.头发换装) {
        $gameSystem._mapActorFairPicture = hzItem.meta.头发换装;
    } else {
    ...
    if (hzItem && hzItem.meta.地图立绘换装) {
        ...
        if (lv == 0) {
            ...
            const dthzImg = hzItem.meta.地图立绘换装 + lv;
...
////////////////////////////Scene//////////////////////////
FlyCat.LL_SceneMenu.Scene_MenuBase_initialize = Scene_MenuBase.prototype.initialize;
...
Scene_MenuBase.prototype.createActorsSprite = function () {
    ...
    if (SceneManager._scene instanceof Scene_Equip) {
        ...
        var x = FlyCat.LL_SceneMenu.pictureX;
    }
    var y = FlyCat.LL_SceneMenu.pictureY;
...
FlyCat.LL_SceneMenu.Scene_MenuBase_update = Scene_MenuBase.prototype.update;
Scene_MenuBase.prototype.update = function () {
    FlyCat.LL_SceneMenu.Scene_MenuBase_update.call(this);
    // /*表情*/
    ...
    //         this._actorMenuSprite.scale.y += 0.0001;
    //         this._actorMenuMainSprite.scale.y += 0.0001;
