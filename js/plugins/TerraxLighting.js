...
* @text 添加到选项
* @desc 将“打开或关闭脚本”添加到“选项”菜单
* Default: Yes
* @default Yes
*
* @param Option menu entry
* @text 选项菜单项
* @desc 选项菜单中使用的文本项（根据不同的语言更改）
...
* @text 存储小时
* @desc 存储在游戏变量中小时的时间（0-23）
* Default: 0
* @default 0
*
* @param Save DaynightMinutes
* @text 存储分钟
* @desc 存储在游戏变量中分钟的时间（0-59）
* Default: 0
* @default 0
*
* @param Save DaynightSeconds
* @text 存储秒数
* @desc 存储在游戏变量中秒数的时间（0-59）
* Default: 0
* @default 0 
*
* @param Flashlight offset
* @text 手电筒偏移
* @desc 增加此设置可向上移动双倍高度的位置
* Default: 0
* @default 0 
*
* @param Screensize X
* @text 屏幕尺寸宽度
* @desc 如果使用比默认值更宽的屏幕分辨率，则增加此值
* Default : 1280
* @default 1280
*
* @param Screensize Y
* @text 屏幕尺寸高度
* @desc 如果使用比默认值更高的屏幕分辨率，则增加此值
* Default : 720
* @default 720
*
* @param Kill Switch
* @text 切断开关
* @desc 独立开关A、B、C、D。如果独立开关 X打开，事件光源将被禁用。
* Default : No
* @default No
*
...
* 3.你搞定了，就这么简单。
* 可以为亮度和方向添加两个可选命令
* Light 200 #FFFFFF B50 将亮度提高50%。值介于0和99之间。
* Light 200 #FFFFFF D1 将为放置在墙上的光源提供半个光球。
* 1.对于北墙上的灯，灯光将朝下。
* 2.对于东墙上的灯，灯光将面向西方。
* 3.对于南墙上的灯，灯光将朝北。
* 4.对于西墙上的灯，灯光将面向东。
...
* 'Light radiusgrow 200 #FFFFFF'（更改半径和颜色）
* 如果你想随着时间的推移慢慢改变玩家的半径（就像一个垂死的火炬）
* 使用“Light radiusgrough 200”命令
* 您可以通过添加以下内容来更改播放机光源的亮度：
* 'Light radius 200 #FFFFFF B70' （亮度值介于0和99之间，默认值为0）
*
* 要在游戏中打开和关闭光源，请执行以下操作：
* 给光源一个普通的 def :  Light 250 #FFFFFF 和一个额外的数字
* 所以它变成了 'Light 250 #FFFFFF 1'
* 如果您使用可选的亮度和方向，请将其置于这些(Light 250 #FFFFFF B50 D2 1)
* 要打开此灯，请使用plugin命令： 'Light on 1'.
* plugin命令还将触发目标灯光上的SelfSwitch'D'。
* 要关闭灯，请使用plugin命令： 'Light off 1'.
* 您可以使用选项或手动方式重置每个地图的开关
* 插件命令'Light switch reset'
* 也可以使用参数中定义的kill self开关关闭灯光。
* 你可以改变有id的光源的颜色。使用插件调用
* 'Light color 1 #FF0000'
*
* 您可以使用下面的note标记让浅色自动循环
* Light cycle #FF0000 50 #00FF00 50 #0000FF 50
* 这将使颜色从红色到绿色再到蓝色，每50毫秒一次。
* 最少2种颜色，最多4种颜色。
*
* 将“Light”键世界替换为“Fire”将使灯光产生微妙的闪烁
* 您可以使用plugin命令配置fire效果 'SetFire 7 10'
* 其中7是半径变化，10是颜色从红色到黄色的变化。
* 
* 完全关闭脚本使用 : 'Light deactivate'
* 要再次打开它，请使用命令: 'Light activate'
*
* 要激活地图上的昼夜循环，请在事件注释中添加带有“DayNight”的触发器
* 或者地图上的注释。
* 插件命令'day night speed 10'改变速度。
* 速度10意味着在游戏中通过一个小时需要10秒（可能是快速）
* 插件命令'Daynight hour 1630'将小时设置为16:30小时
* 每个小时都有自己的颜色值.
* 插件指令 'Daynight color 0 #222222' 将0:00小时更改为颜色值 '#222222'
* 你可以用插件命令'Daynight add 830'添加时间（这会增加8小时30分钟
*
* 如果你想利用一天中的时间来触发效果（比如在天黑时打开灯）
* 您可以使用参数“Save DaynightHours”、“Save DaynightMinutes”、“Save DaynightSeconds”
* 默认值为0，表示关闭。
* 如果将其设置为一个值，例如5，它将把daynight值存储在游戏变量5中。
* 然后可以使用该变量来触发灯光。
* 为了帮助同步/调试时间系统，可以使用scriptcommand“daynight debug”来显示当前时间。
* 如果你想去“外星人世界”之类的，你可以用
* 脚本命令“daynight hoursinday 48”（对于一天中的48小时，不要忘记填写小时值）
*
* 作为昼夜循环的替代方案，您可以使用色调系统。如果你想使用另一个插件
* 白天/晚上循环，色调选项可能是最好的使用。
* 插件命令'Tint set#333333'将使房间不那么黑暗。
* 插件命令'Tint fade#777777 5'将从当前颜色淡入新颜色，最后一种颜色
* 数字（5）是淡出的速度，1是快速淡出，20是非常慢的。
* 如果某个区域有昼夜循环系统，则色调系统将被禁用。
*
* 要使用手电筒效果，请使用“手电筒打开8 12#FFFFFF 3”和“手电筒关闭”
* 参数是可选的（8=光束长度，12=光束宽度，#FFFFFF=颜色，3=光束密度）
* 事件也可以使用闪光灯效果。在便笺标签中使用“lashlight 8 12 #8888881 2”。
* 其中8是手电筒光束的长度，12是光束的宽度。最后的数字是
* 可选，可用于打开或关闭NPC的手电筒并设置光束方向
* 如果事件没有移动（1=向上，2=右，3=下，4=左），则默认为向下。
*
* TileLight和RegionLight设置
* 要在不使用事件的情况下创建光源，可以使用以下插件命令。
* “TileLight 1 ON #FFFFFF 150”都将创建一个“#FFFFFF radius 150”颜色的平铺。
* FFFFFF 150上的TileRegion 1将在区域编号为1的所有瓷砖上创建光源。
* 您可以使用#FFFFFF 150 B50上的可选平铺区域1来增加光源的亮度（亮度增加50%）
* TileLight 1关闭将再次关闭TileLight tag 1上的灯光
* TileRegion 1 OFF将再次关闭区域1上的灯
* TileFire和RegionFire的工作原理与TileLight相同，但具有火效果。
* TileGlow和RegionGlow的工作原理与TileLight相同，但具有轻微的脉冲效果。

* 确保映射中至少还有一个事件包含灯光，否则脚本将无法运行。
*
* TileBlock和RegionBlock设置
* 要阻止某些瓷砖（例如屋顶）上的灯光，可以使用以下插件命令。
* TileBlock 1 ON #000000  将阻止具有平铺标记1的瓷砖上的灯光。
* RegionBlock 1 ON #000000 将阻止区域编号为1的瓷砖上的灯光。
* TileBlock 1 OFF and TileRegion 1 OFF 将再次关闭阻塞。.
* 例如，要使光线变暗但不能完全阻挡光线，请使用稍高的颜色设置（#333333）。
* 光线跟踪功能不能。如果玩家的光环半径更大，那么阻拦会平铺
* 光线会在另一边出现。为了获得最佳效果，请使“光半径”比“块”部分小一点。
* 对于高级用户，如果要阻止更多或更少的磁贴，可以执行以下操作
* RegionBlock 1 ON #000000 形状X偏移Y偏移宽度高度
* RegionBlock 1 ON #000000 1 20 20 10 10   -> 这将阻止一个从20,20开始，宽度和高度为10,10的盒子
* RegionBlock 1 ON #000000 2 20 20 10 10   -> 这将阻止一个椭圆从20，20开始，x半径10和yradius 10
*
* 你可以使用下面的插件调用来创建爆炸或咒语的短时光效果。
* plugin command: effect_on_event 003 200 #FFAAAA 50
* -> 将在事件上创建一个lightglobe，id 003，半径200，颜色为ffaaa，持续时间为50帧
* plugin command: effect_on_xy 560 500 50 #FFAAAA 25
* -> 将在坐标560，500，半径50，颜色为ffaaa，持续25帧的光球s
* 可以添加一些特殊效果，在手册中有解释
* Terrax照明系统与Moghunter时间系统兼容，详情请参阅自述。
*
* 根据麻省理工学院的许可证，
* 如果用于商业项目，请随意捐款或
* 更好的是，给我一个你创造的免费版本
* 邮箱 : fox(AT)caiw.nl / terraxz2 on steam.
* 
* 感谢rpgmakerweb社区中所有人对idea的支持和兴趣。
* 特别感谢Galv解决了一些问题。
*/
//=============================================================================
...
(function () {
	...
	PluginManager.registerCommand('TerraxLighting', 'pluginCommand', args => {
		const command = args.command.split(' ');
		const commandF = command.shift();
		Game_Party.prototype.pluginCommand(commandF, command)
	});

	//	var _Game_Interpreter_pluginCommand = Game_Interpreter.prototype.pluginCommand;
	Game_Party.prototype.pluginCommand = function (command, args) {
