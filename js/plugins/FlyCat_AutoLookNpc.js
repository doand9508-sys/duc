...
Scene_Map.prototype.createAllWindows = function () {
    ...
    this._timeSprite_1.bitmap = ImageManager.loadPicture('xunlu' + img);
    this._timeSprite_1.x = 10;
    this._timeSprite_1.y = 106;
    this._timeSprite_2 = new Sprite();
    this.addChild(this._timeSprite_2)
    this._timeSprite_2.bitmap = ImageManager.loadPicture('autolook' + img);
...
Game_System.prototype.moveToCharacter = function (npcEvent) {
    ...
    if (bestCharacter) {
        ...
        //wolfzq追加，在system中记录位置，抵达位置后，如果处于飞行状态会落地，并触发事件。
        if ($gameSwitches.value(19)) {
            this._flyNpc = [bestCharacter.x, bestCharacter.y];
        }
    }
}
