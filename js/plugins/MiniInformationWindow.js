...
Window_MiniInfo.prototype.makeContents = function () {
    ...
    if (item.traits) {
        for (var i = 0, max = item.traits.length; i < max; i++) {
            ...
            var is100Plus = Math.floor(Math.abs(value < 0 || value == 1 ? value * 1000 / 10 : (1000 - value * 1000) / 10));
            text = '';
            switch (trait.code) {
                case 111:
                case 11:
                ...
                case 32:
                    if (DrillUp.m_attr_atk1_visible === "true" && value > 0) {
                        var state = $dataStates[dataId].name;
                        text = s + DrillUp.m_attr_atk1_text + c + state + ' ' + is100Plus + '%';
                    }
                    break;
                case 33:
                ...
                case 61:
                    if (DrillUp.m_attr_move_visible === "true" && value > 0) {
                        text = s + DrillUp.m_attr_move_text + du + sym + is100Plus + '%';
                    }
                    break;
                case 62:	//特殊标志
