...
/*:
 * @target MZ
 * @plugindesc v1.0.0 FlyCat-<跨存档变量/开关>
 * @author FlyCat
 * 
 * @param saveVariables
 * @text 变量设置
...
DataManager.LoadVariables = function () {
    if (ConfigManager._saveVariables) {
        ...
        for (let i = 0; i < max; i++) {
            ...
            /*wolfzq取消回想使用插件
			if (id == 1000) {
                var value = ConfigManager._saveVariables[id];
            } else {*/
                var value = Number(ConfigManager._saveVariables[id]);
            //}
