...
Window_ItemPromt.prototype.refresh = function (item) {
    ...
    if (this._item == 'gold') {
        if (this._itemNumber > 0) {
            var text = '\\C[14]Gain Gold:';
            SoundManager.playItemPromtSe('Coin')
            this.changeTextColor(ColorManager.textColor(14));
        } else {
            var text = '\\C[3]Lose Gold:';
            SoundManager.playItemPromtSe('Shop2')
            this.changeTextColor(ColorManager.textColor(3));
        }
        this.drawTextEx(text + '\\I[' + Number(FlyCat.ItemPrompt.goldIcon) + ']' + this._itemNumber, x - this.textWidth(text + this._itemNumber) / 2 + 16, y, this.width);
    } else {
        if (this._item == 'variable') {
            ...
            if (this._itemNumber > 0) {
                var text = '\\C[14]Gain Item:';
                var text_1 = ' x';
                SoundManager.playItemPromtSe('Chime2')
                this.drawTextEx(text + '\\I[' + this._item.iconIndex + ']' + this._item.name + text_1 + this._itemNumber, x - this.textWidth(text + this._item.name + text_1 + this._itemNumber) / 2 + 16, y, this.width);
            } else {
                var text = '\\C[3]Lose Item:';
                var text_1 = '';
                SoundManager.playItemPromtSe('Equip1')
                this.drawTextEx(text + text_1 + '\\I[' + this._item.iconIndex + ']' + this._item.name + 'x' + Math.abs(this._itemNumber), x - this.textWidth(text + this._item.name + 'x' + Math.abs(this._itemNumber)) / 2 + 16, y, this.width);
            }
        }
    }
	//古币特殊处理，如果后续还有新的物品，再改for循环判断
	if (this._item == $dataItems[60]) {
		this._vSpeed = 30;
	} else {
		this._vSpeed = FlyCat.ItemPrompt.speed;
	}
    this.show();
};

Window_ItemPromt.prototype.resetFontSettings = function () {
    this.contents.fontFace = $gameSystem.mainFontFace();
    this.contents.fontSize = 24;
    this.resetTextColor();
};

Bitmap.prototype.catFillRect = function (
    x, y, width, height
) {
    const context = this.context;
    var gradient = context.createLinearGradient(x, y, x + width, y + height);
    gradient.addColorStop(0, "rgba(0, 0, 0, 0)"); // 左边透明
    gradient.addColorStop(0.5, "rgba(0, 0, 0, 0.7)"); // 中间不透明
    gradient.addColorStop(1, "rgba(0, 0, 0, 0)"); // 右边透明
    context.save();
...
Window_ItemPromt.prototype.drawBack = function (x, y) {
    ...
    this.contents.catFillRect(wx, wy, w, h);
};
    ...
    if (this._playItemPrompt) {
        if (this.visible && this._item) {
            if (this._itemPromptSprite.x < this.x - 20) {
                ...
                if (this._startCount > this._vSpeed) {
                    this._item = null;
                    this._startCount = 0;
                }
            }
...
//-----------------获得物品提示加速-------------
Window_ItemPromt.prototype.isSkip = function() {
