...
PluginManager.registerCommand('FlyCat_Letter', 'openLetterWindow', args => {
    //   SceneManager.push(Scene_Letter);2023.9.23
...
Scene_Map.prototype.openLetter = function () {
    if (Imported.Cat_Children_X && this._newChildBackSprite && this._newChildBackSprite.visible) {

    } else {
        SoundManager.playOk();
        SceneManager.push(Scene_LetterNpc);
    };
    // if ($gameTemp._setNpcText) {2023.9.23
    //     SceneManager.push(Scene_LetterNpc);
    // } else {
...
Game_System.prototype.LetterPlayerHd = function (text, item) {//正常对话
    ...
    var include = data.includes(text);
    if (include == true) return data_1[Math.floor((Math.random() * data_1.length))];
    /*通用类型*/
    const data1 = ['你好', 'hello', 'hi', '好'];//打招呼类型
    const data1_1 = ['母亲大人\n\\C[14]安好', '母亲大人好', '母亲，有什么要事吗？', '母亲大人，难得找孩儿。'];//孩子回复话术
...
Scene_Letter.prototype.createChlidrenListWindow = function () {
    ...
    if (this._chListWindow._list.length > 0) {
        if ($gameTemp._setCId) {
            this._chListWindow.select($gameTemp._setCId);
            const pagination = Math.floor(($gameTemp._setCId + 1) / 8);
            this._chListWindow.smoothScrollDown(pagination * 8);
            $gameTemp._setCId = null;
        } else {
            this._chListWindow.select(0);
        }
        this._chListWindow.activate();
...
/*
Scene_Letter.prototype.createCancelButton = function () {
...
Window_LetterInfo.prototype.drawBackgroundRect = function (rect) {
};
Window_LetterInfo.prototype.drawNpcImg = function (bitmap, rect, scaleX, scaleY) {
...
Window_LetterInfo.prototype.LoadNpcBitmap = function (bitmap, index, meta) {
...
Window_LetterChildrenList.prototype.refresh = function () {
    ...
    //     $gameSystem._LetterData = [];
    // }
    if ($gameSystem._LetterData.length > 2) $gameSystem._LetterData.sort((a, b) => { return a.dead - b.dead })
    this.contents.clear();
    this.contentsBack.clear();
...
Window_LetterChildrenList.prototype.drawItem = function (index) {
    ...
    if (children) {
        ...
        if (children.dead == 1) {
            ...
            this.drawText(children.name + '（已死亡）', rect.x, rect.y, this.width, 'left')
        } else {
...
PluginManager.registerCommand('FlyCat_Letter', 'setChildrenText', args => {//2023.9.23
    // const id = Number(args.id) - 1;
    // const text = args.text;
    // var item = null;
    // if (args.item) {
    //     var item = JsonEx.parse(args.item);
    // }
    // if ($gameSystem._LetterData[id] && text) {
    //     $gameSystem.playLetterChildrenMessage(text, $gameSystem._LetterData[id], -1, args.img, Number(args.max), Number(args.scaleX), Number(args.scaleY), item);
    //     $gameParty.ssLetter(true);
    //     $gameTemp._setCId = id;
    // };
});

PluginManager.registerCommand('FlyCat_Letter', 'removeChildren', args => {
    const id = Number(args.id) - 1;
    if ($gameSystem._LetterData && $gameSystem._LetterData[id]) {
        const actor = JsonEx.makeDeepCopy($gameSystem._LetterData[id]);
        actor.dead = 1;
        $gameSystem._LetterData.splice(id, 1);
        $gameSystem._LetterData.push(actor);
        if ($gameSystem._LetterData.length > 2) $gameSystem._LetterData.sort((a, b) => { return a.dead - b.dead })
    };
});

Game_System.prototype.playLetterChildrenMessage = function (message, item, type, img, max, scaleX, scaleY, items) {
    var htime = new Date();
    var hour = htime.getHours();   //时
    var min = htime.getMinutes();  //分
    var ss = htime.getSeconds();   //秒
        ...
        if (items) {
            ...
            for (let i = 0; i < list.length; i++) {
                if (list[i]) {
                    ...
                    //  console.log('获得 ' + $dataItems[id].name);
                }
            }
...
function 检测指定序号孩子是否存在(id) {
    if ($gameSystem._LetterData[id - 1]) {
        return true;
    }
    return false;
};

function 获取指定序号孩子好感度(id) {
    if ($gameSystem._LetterData[id - 1]) {
        return Number($gameSystem._LetterData[id - 1].haoGanDu);
    };
};

function 获取指定序号孩子性格(id) {
    if ($gameSystem._LetterData[id - 1]) {
        return $gameSystem._LetterData[id - 1].xingGe;
    }
};

function 获取指定序号孩子男女(id) {
    if ($gameSystem._LetterData[id - 1]) {
        return $gameSystem._LetterData[id - 1].xingBie;
    }
};