...
//每秒清除一行日志
Window_BattleLog.prototype.clearNextLine = function() {
	if (this._lines && this._lines.length > 0) {
		this._waitLine ++ ;
		if (this._waitLine > 60) {
			this._waitLine = 0;
			this._lines.shift(); // 删除第一行
			this.refresh();     // 刷新显示
		}
	} else {
		this._waitLine = 0;
...
Window_BattleLog.prototype.stopAllLines = function() {
	this._waitLine = undefined;
	this.clear();
}
